...
import { LanguageInput, ShikiTransformer, type ThemeRegistrationAny } from '@shikijs/types';

import anchorPlugin from 'markdown-it-anchor';
import mathPlugin from 'markdown-it-mathjax3';

import { MarkdownItAsync, Options } from 'markdown-it-async';
import { full as emojiPlugin } from 'markdown-it-emoji';
import attrsPlugin from 'markdown-it-attrs';
import { BuiltinLanguage, BuiltinTheme, Highlighter } from 'shiki';

import type { Awaitable } from './shared/shared';
...
export async function createMarkdownRenderer(srcDir: string, options: MarkdownOptions = {}, base = '/', logger = console) {
  if (md) {
    disposeMdItInstance();
  }
  ...
  let [highlight, dispose] = options.highlight ? [options.highlight, () => {}] : await createHighlighter(theme, options, logger);
