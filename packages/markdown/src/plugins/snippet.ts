...
export const snippetPlugin = (md: MarkdownItAsync, srcDir: string) => {
  const parser: RuleBlock = (state, startLine, endLine, silent) => {
    ...
    // 这里没有相关环境取不到路径，将 srcDir 和 filepath 拼接
    const resolvedPath = (realPath || _path) ? path.resolve(path.dirname(realPath || _path), filepath) : path.join(srcDir, filepath);
