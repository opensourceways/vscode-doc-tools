...
const onAsyncTaskOutput = (name: string, extras: any) => {
  if (name === 'batchCheckTagClosed:stop') {
    working.value = false;
    currentScanning.value = '';
  } else if (name === 'batchCheckTagClosed:scanTarget') {
    working.value = true;
    currentScanning.value = extras;
  } else if (name === 'batchCheckTagClosed:addItem') {
...
const onClickStartLink = () => {
  data.value = [];
  Bridge.getInstance().broadcast('asyncTask:batchCheckTagClosed', injectData.extras?.fsPath);
};

const onClickStopLink = () => {
  Bridge.getInstance().broadcast('asyncTask:stopBatchCheckTagClosed');
};
</script>
...
<template>
  <div class="check-result">
    ...
    <div class="text">【检查结果】：共检查出 <span class="red">{{ data.length }}</span> 个未闭合标签</div>
    <div class="text">
      <span>【控制开关】：</span>
    ...
    <OTable :columns="columns" :data="data" border="all">
      <template #td_file="{ row }">
        <OLink color="primary" @click="onClickSourceLink(row)">{{ row.file }}</OLink>
      </template>
